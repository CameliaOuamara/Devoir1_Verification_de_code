...
N = 400                                              # Nombre de points de discretisation [-]
...
delta_t = 0.01 * 0.5 * delta_r*delta_r / D_eff              # Pas de temps [s]
...
# # ----------------------------------------------------------------------------------------
# #       MMS : Schéma 1 : Discretisation d'ordre 1 en temps et 1 en espace étude ordre spatial
# # ----------------------------------------------------------------------------------------
...
# #%%
# # ----------------------------------------------------------------------------------------
# #          MMS : Schéma 2 : Discretisation d'ordre 1 en temps et 2 en espace étude ordre spatial
# # ----------------------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------------------
# #       MMS : Schéma 1 : Discretisation d'ordre 1 en temps et 1 en espace étude ordre temporel
# # ----------------------------------------------------------------------------------------
...
# #%%
# # ----------------------------------------------------------------------------------------
# #          MMS : Schéma 2 : Discretisation d'ordre 1 en temps et 2 en espace étude ordre temporel
# # ----------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------------------------------
#                           Solution avec le maillage le plus fin (pour MNP), Schéma 1, étude spatial
# -----------------------------------------------------------------------------------------------------------------
Deff = 1.0e9
k = 4.0e9
N_vect = np.array([10,20,30,40,50])
delta_r_vect = R/(N_vect-1)
delta_t_MNP = 0.01 * 0.5 * delta_r_vect*delta_r_vect / Deff
sol_MNP_spatial = Profil_Concentration(delta_r, delta_t_MNP[-1], N, R, critere_convergence, critere_max_iter, Deff, k)
sol_MNP_spatial.Algorithme_Resolution()
nb_time_step = sol_MNP_spatial.C.shape[0]
r_fine_mesh_spatial = np.linspace(0, R, sol_MNP_spatial.C.shape[1])
t_fine_mesh_spatial = np.linspace(0, delta_t_MNP[-1]*(nb_time_step-1), nb_time_step)
spline_bicubic_spatial = sp.interpolate.RectBivariateSpline(t_fine_mesh_spatial, r_fine_mesh_spatial, sol_MNP_spatial.C[:,:])


# -----------------------------------------------------------------------------------------------------------------
#                           Solution avec le maillage le plus fin (pour MNP), Schéma 2, étude spatial
# -----------------------------------------------------------------------------------------------------------------
sol_MNP_Centree_spatial = Profil_Concentration_Centree(delta_r, delta_t_MNP[-1], N, R, critere_convergence, critere_max_iter, Deff, k)
sol_MNP_Centree_spatial.Algorithme_Resolution()
nb_time_step = sol_MNP_Centree_spatial.C.shape[0]
r_fine_mesh_Centree_spatial = np.linspace(0, R, sol_MNP_Centree_spatial.C.shape[1])
t_fine_mesh_Centree_spatial = np.linspace(0, delta_t_MNP[-1]*(nb_time_step-1), nb_time_step)
spline_bicubic_Centree_spatial = sp.interpolate.RectBivariateSpline(t_fine_mesh_Centree_spatial, r_fine_mesh_Centree_spatial, sol_MNP_Centree_spatial.C[:,:])

# ----------------------------------------------------------------------------------------
#       MNP : Schéma 1 : Discretisation d'ordre 1 en temps et 1 en espace étude ordre spatial
# ----------------------------------------------------------------------------------------

plt.figure(0)

t_final = t_fine_mesh_spatial[-1]

Objet_Etude_Convergence = Etude_Convergence_MNP_spatial(delta_r_vect, delta_t_MNP[-1], N_vect, R, t_final, 1, spline_bicubic_spatial)
erreur_vect_L1, erreur_vect_L2, erreur_vect_L_inf = Objet_Etude_Convergence.Boucle_iterations()
...
plt.legend()
plt.grid()
plt.title("Normes des erreurs L1, L2 et $L_\infty$ avec schéma d'ordre 1 en \n fonction de $\Delta$r avec méthode MNP")
plt.savefig("Norme_des_erreurs_Schema_1_MMS_Spatial.png")
plt.show()
...
#%%
# ----------------------------------------------------------------------------------------
#          MNP : Schéma 2 : Discretisation d'ordre 1 en temps et 2 en espace étude ordre spatial
# ----------------------------------------------------------------------------------------

plt.figure(2)

t_final = t_fine_mesh_Centree_spatial[-1]
Objet_Etude_Convergence = Etude_Convergence_MNP_spatial(delta_r_vect, delta_t_MNP[-1], N_vect, R, t_final, 2, spline_bicubic_Centree_spatial)
erreur_vect_L1_Centree, erreur_vect_L2_Centree, erreur_vect_L_inf_Centree = Objet_Etude_Convergence.Boucle_iterations()    
...
# -----------------------------------------------------------------------------------------------------------------
#                           Solution avec le maillage le plus fin (pour MNP), Schéma 1, étude temporel
# -----------------------------------------------------------------------------------------------------------------
Deff = 1.0e-9
k = 4.0e-9
delta_t_MNP = 0.01 * 0.5 * delta_r_vect*delta_r_vect / Deff
sol_MNP_temporel = Profil_Concentration(delta_r, delta_t_MNP[-1], N, R, critere_convergence, critere_max_iter, Deff, k)
sol_MNP_temporel.Algorithme_Resolution()
nb_time_step = sol_MNP_temporel.C.shape[0]
r_fine_mesh_temporel = np.linspace(0, R, sol_MNP_temporel.C.shape[1])
t_fine_mesh_temporel = np.linspace(0, delta_t_MNP[-1]*(nb_time_step-1), nb_time_step)
spline_bicubic_temporel = sp.interpolate.RectBivariateSpline(t_fine_mesh_temporel, r_fine_mesh_temporel, sol_MNP_temporel.C[:,:])


# -----------------------------------------------------------------------------------------------------------------
#                           Solution avec le maillage le plus fin (pour MNP), Schéma 2, étude temporel
# -----------------------------------------------------------------------------------------------------------------
sol_MNP_Centree_temporel = Profil_Concentration_Centree(delta_r, delta_t_MNP[-1], N, R, critere_convergence, critere_max_iter, Deff, k)
sol_MNP_Centree_temporel.Algorithme_Resolution()
nb_time_step = sol_MNP_Centree_temporel.C.shape[0]
r_fine_mesh_Centree_temporel = np.linspace(0, R, sol_MNP_Centree_temporel.C.shape[1])
t_fine_mesh_Centree_temporel = np.linspace(0, delta_t_MNP[-1]*(nb_time_step-1), nb_time_step)
spline_bicubic_Centree_temporel = sp.interpolate.RectBivariateSpline(t_fine_mesh_Centree_temporel, r_fine_mesh_Centree_temporel, sol_MNP_Centree_temporel.C[:,:])


# ----------------------------------------------------------------------------------------
#       MNP : Schéma 1 : Discretisation d'ordre 1 en temps et 1 en espace étude ordre temporel
# ----------------------------------------------------------------------------------------

plt.figure(4)
delta_t_MNP = delta_t_MNP*8
t_final = t_fine_mesh_temporel[-1]
Objet_Etude_Convergence = Etude_Convergence_MNP_temporel(delta_r, delta_t_MNP, N, R, t_final, 1, spline_bicubic_temporel)
erreur_vect_L1, erreur_vect_L2, erreur_vect_L_inf = Objet_Etude_Convergence.Boucle_iterations()

plt.figure(5)

# Graphique log-log norme de l'erreur L1 vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L1, '.r', label = "Norme L1")

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L1), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
...
# Extrapoler la valeur prédite pour la dernière valeur de h_values
extrapolated_value = fit_function(delta_t_MNP[-1])
plt.loglog(delta_t_MNP, fit_function(delta_t_MNP), linestyle='--', color='r')
...
# Graphique log-log norme de l'erreur L2 vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L2, '.g', label = "Norme L2")

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L2), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
...
# Extrapoler la valeur prédite pour la dernière valeur de h_values
extrapolated_value = fit_function(delta_t_MNP[-1])
plt.loglog(delta_t_MNP, fit_function(delta_t_MNP), linestyle='--', color='g')
...
# Graphique log-log norme de l'erreur Linf vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L_inf, '.m', label='Norme $L_\infty$')

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L_inf), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
...
# Extrapoler la valeur prédite pour la dernière valeur de h_values
extrapolated_value = fit_function(delta_t_MNP[-1])
plt.loglog(delta_t_MNP, fit_function(delta_t_MNP), linestyle='--', color='m')
...
plt.legend()
plt.grid()
plt.title("Normes des erreurs L1, L2 et $L_\infty$ avec schéma d'ordre 1 en \n fonction de $\Delta$t avec méthode MNP")
plt.savefig("Norme_des_erreurs_Schema_1_MMS_Temp.png")
plt.show()

#%%
# ----------------------------------------------------------------------------------------
#          MNP : Schéma 2 : Discretisation d'ordre 1 en temps et 2 en espace étude ordre temporel
# ----------------------------------------------------------------------------------------

plt.figure(6)

t_final = t_fine_mesh_Centree_temporel[-1]
Objet_Etude_Convergence = Etude_Convergence_MNP_temporel(delta_r, delta_t_MNP, N, R, t_final, 2, spline_bicubic_Centree_temporel)
erreur_vect_L1_Centree, erreur_vect_L2_Centree, erreur_vect_L_inf_Centree = Objet_Etude_Convergence.Boucle_iterations()    

plt.figure(7)

# Graphique log-log norme de l'erreur L1 vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L1_Centree, '.r', label = "Norme L1")

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L1_Centree), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
...
# Graphique log-log norme de l'erreur L2 vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L2_Centree, '.g', label = "Nomre L2")

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L2_Centree), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
...
# Graphique log-log norme de l'erreur Linf vs delta_t
plt.loglog(delta_t_MNP, erreur_vect_L_inf_Centree, '.m', label = "Norme $L_\infty$")

# Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
coefficients = np.polyfit(np.log(delta_t_MNP), np.log(erreur_vect_L_inf_Centree), 1)
exponent_logreg = coefficients[0]
constant_logreg = coefficients[1]
